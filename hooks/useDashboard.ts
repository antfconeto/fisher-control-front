...
export const useDashboard = () => {
  ...
  const getAdditionalStats = () => {
    const totalCapacity = tankStatus.reduce(
      (sum, tank) => sum + tank.capacity,
      0
    );
    const totalOccupation = tankStatus.reduce(
      (sum, tank) => sum + (tank.capacity * tank.occupation) / 100,
      0
    );
    const averageTemperature =
      tankStatus.reduce((sum, tank) => sum + tank.temperature, 0) /
      tankStatus.length;
    const averagePh =
      tankStatus.reduce((sum, tank) => sum + tank.ph, 0) / tankStatus.length;
    ...
    return {
      ...
      maintenanceTanks: tankStatus.filter(
        (tank) => tank.status === "maintenance"
      ).length,
      warningTanks: tankStatus.filter((tank) => tank.status === "warning")
        .length,
    };
  };
