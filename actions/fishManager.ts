...
export async function requestToJoinFishManager(
  ...
  try {
    ...
    console.info(
      `✅ Successfully requested to join Fish Manager: ${fishManagerId}`
    );
    return result;
  } catch (error: any) {
    console.error(`❌ Error in requestToJoinFishManager: ${error.message}`);
    throw new CustomError(
      getFishManagerErrorMessage("requestToJoin", error.statusCode || 500),
      error.statusCode || 500
    );
  }
}

function getFishManagerErrorMessage(
  context: string,
  statusCode: number
): string {
  const fishManagerErros = errorMessages.fishManager as any;
  return fishManagerErros[context][statusCode] || "Erro desconhecido.";
}
