...
export const createSpecie = async (specie:Specie): Promise<Specie | ResponseError> => {
    ...
    try {
        ...
        consoler.error(
            `Error to create specie, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getSpecieErrorMessage('createSpecie', error.statusCode || 500), error.statusCode || 500);
    }
};

export const deleteSpecie = async (specieId:string): Promise<boolean | ResponseError> => {
    ...
    try {
        const response = await fetch(`${urlApi}/specie/deleteSpecie?specieId=${specieId}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token!.value}`,
        ...
        // Captura de erros em qualquer ponto
        consoler.error(
            `Error to delete specie, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getSpecieErrorMessage('deleteSpecie', error.statusCode || 500), error.statusCode || 500);
    }
};

export const updateSpecie = async (specie:Specie): Promise<Specie | ResponseError> => {
    ...
    try {
        ...
        // Captura de erros em qualquer ponto
        consoler.error(
            `Error to update specie, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getSpecieErrorMessage('updateSpecie', error.statusCode || 500), error.statusCode || 500);
    }
};
...
export const getAllSpecies = async (): Promise<Specie[] | ResponseError> => {
    ...
    try {
        ...
        // Captura de erros em qualquer ponto
        consoler.error(
            `Error to get all specie, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getSpecieErrorMessage('getAllSpecies', error.statusCode || 500), error.statusCode || 500);
    }
};

export const getAllQuantityAnimalsFromSpecie = async (specieId:string): Promise<number | ResponseError> => {
    console.log(`🔁 Initing process to get all specie`);
    //get token from cookie
    const token = (await cookies()).get('access_token')
    if (!token) {
    ...
    try {
        const response = await fetch(`${urlApi}/animal/getAllQuantityAnimalsFromSpecie`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token!.value}`,
        ...
        if (!response.ok) {
            ...
            console.log(errorMessage)
            return {
                error: getSpecieErrorMessage('getAllQuantityAnimalsFromSpecie', response.status),
                statusCode: response.status,
            };
        }
        //get info as json
...
export const getSpecieById = async (specieId:string): Promise<Specie | ResponseError> => {
    ...
    try {
        ...
        // Captura de erros em qualquer ponto
        consoler.error(
            `Error to retrieve specie, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getSpecieErrorMessage('getSpecieById', error.statusCode || 500), error.statusCode || 500);
    }
};
