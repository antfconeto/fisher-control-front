...
export const loginAction = async (
  ...
  consoler.process(
    `🔁 Iniciando processo de login para o usuário com email: ${data.email}`
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      consoler.error(
        `Erro ao fazer login para o usuário, erro: ${JSON.stringify(
          errorMessage
    ...
    throw new CustomError(
      getUserErrorMessage("login", error.statusCode || 500),
