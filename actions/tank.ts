...
export const createTank = async (tank: Tank): Promise<Tank | ResponseError> => {
    ...
    try {
        ...
        consoler.error(
            `Error to create tank, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getTankErrorMessage('createTank', error.statusCode || 500), error.statusCode || 500);
    }
};
...
export const updateTank = async (tank: Tank): Promise<Tank | ResponseError> => {
    ...
    try {
        ...
        consoler.error(
            `Error to update tank, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getTankErrorMessage('updateTank', error.statusCode || 500), error.statusCode || 500);
    }
};

export const deleteTank = async (tankId: string): Promise<void | ResponseError> => {
    ...
    try {
        ...
        consoler.error(
            `Error to delete tank, error: ${error.message}, statusCode: ${error.statusCode || 500}`
        );
        throw new CustomError(getTankErrorMessage('deleteTank', error.statusCode || 500), error.statusCode || 500);
    }
};
