...
export const createAnimal = async (
  ...
  try {
    ...
    throw new CustomError(
      getAnimalErrorMessage("createAnimal", error.statusCode || 500),
      error.statusCode || 500
    );
  }
};
...
export const updateAnimal = async (
  ...
  try {
    ...
    // Captura de erros em qualquer ponto
    consoler.error(
      `Error to update animal, error: ${error.message}, statusCode: ${
        error.statusCode || 500
      }`
    );
    throw new CustomError(
      getAnimalErrorMessage("updateAnimal", error.statusCode || 500),
      error.statusCode || 500
    );
  }
};
...
export const listAnimals = async (
  ...
  try {
    ...
    consoler.error(
      `Error fetching animals, error: ${error.message}, statusCode: ${
        error.statusCode || 500
      }`
    );
    throw new CustomError(
      getAnimalErrorMessage("listAnimals", error.statusCode || 500),
      error.statusCode || 500
    );
  }
};
